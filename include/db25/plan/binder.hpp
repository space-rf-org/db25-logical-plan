...
private:
    ...
    // (name -> CTEDefinition node), innermost last. A FROM reference to one of
    // these names binds a fresh copy of the definition's body as a derived table
    // instead of resolving a base table (bind_table_ref checks this before the
    // catalog, so a CTE shadows a same-named table); the definition node also
    // carries the optional column-list rename `WITH t(a, b)`. bind_select
